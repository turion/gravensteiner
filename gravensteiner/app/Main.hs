...
import Data.Map.Strict (Map, alter, toList)
...
updateDirichletColours Colours {yellow, red, green, brown} DirichletColoursPrior {yellowPrior, redPrior, greenPrior, brownPrior, pseudocount} =
  DirichletColoursPrior
...
dirichletColoursLikelihood
  DirichletColoursPrior {yellowPrior, redPrior, greenPrior, brownPrior, pseudocount}
  DirichletColours
    { yellowDirichlet
    , redDirichlet
    , greenDirichlet
...
sampleDirichletColours prior@DirichletColoursPrior {yellowPrior, redPrior, greenPrior, brownPrior} = do
  ...
  samples <- replicateM quality $ do
    ...
    pure
      ( dirichletColours
      , dirichletColoursLikelihood prior dirichletColours
  ...
  unweighted $ proper $ fromWeightedList $ pure samples
...
updateAppleSortPrior Apple {colours} AppleSortPrior {sortColours, frequency} =
  AppleSortPrior
...
identify Apple {colours = Just colours} Model {getModel} = do
  ...
  unweighted $ proper $ fromWeightedList $ pure $ toList likelihoods
...
main = do
  ...
  if dirichletColoursNormalizable initialSortColours then pure () else error "Initial sort colours bad"
