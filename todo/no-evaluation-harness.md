...
- **Reference trees are ground truth.** A documented tree — nursery invoice, gene-bank accession,
  variety collection — has `z_t` observed, so held-out reference trees give a real accuracy figure
  without a pomologist adjudicating anything. This is one more reason the reference-tree provenance
  field in Tier 1 of [the review](model-v1-review.md) is the highest-value single addition.
